...
for metric, data_q, data_ref, ylabel, tag in [
    ("era5", re,  r32, "Lat-wtd RMSE vs ERA5", "rmse_vs_era5"),
    ...
    for ax, v in zip(axes, MODEL_VARS):
        d = data_q if metric=="fp32" else data_q
        if not d[v]: ax.set_visible(False); continue
        m, s = agg(d[v])
    ...
    fig.suptitle(f"DLWP {ylabel} — {args.config} | {len(EVAL_DATES)} dates | mean ±1σ",
                 fontweight="bold")
...
print(f"\nHardware metrics saved: {csv_path}")
