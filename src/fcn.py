...
- IC:    GFS analysis (unchanged, matches FCN's native input)
- Truth: GFS analysis for RMSE (self-consistent forecast error)
         ERA5 (ARCO) for ACC (matches WB2 ERA5 climatology → clean anomalies)
- Climatology: WB2 ERA5-based
...
import torch, zarr
from earth2studio.data import GFS, ARCO, WB2Climatology
...
# ═══════════════════════════════════════════════════════════════
VARIABLES   = ["z500", "z1000", "t850", "t2m", "tcwv", "u10m", "v10m"]
...
gfs     = GFS()                # IC and RMSE truth
era5    = ARCO()               # ERA5 truth ONLY for ACC anomaly
clim    = WB2Climatology()
...
def benchmark(m, handle, n_runs=5):
    ...
    Note: GFS fetches the initial condition inside run.deterministic, so the
    timed region still contains network I/O. Pre-caching ICs locally would make
    ...
    with torch.no_grad():
        run.deterministic([init], NSTEPS, m, gfs, ZarrBackend())
    ...
    for s in range(NSTEPS + 1):
        ...
        try:
            ...
            for v in MODEL_VARS:
                ...
                # WB2 has 721 lats, FCN has 720 — trim south pole
...
def get_era5_truth(date_str):
    """ERA5 truth field at each verification time, for ACC anomaly."""
    init = datetime.fromisoformat(date_str)
    out  = {v: [] for v in MODEL_VARS}
    for s in range(NSTEPS + 1):
        vt = init + timedelta(hours=s*6)
        try:
            da = era5(time=vt, variable=MODEL_VARS)
            for v in MODEL_VARS:
                field = np.squeeze(da.sel(variable=v).values)
                # ARCO has 721 lats, FCN has 720 — trim south pole
                if field.shape[0] == 721 and w_lat.shape[1] == 720:
                    field = field[:720]
                out[v].append(field)
        except Exception as e:
            print(f"  ERA5 truth fetch failed at {vt}: {e}")
            return None
    return {v: np.stack(out[v], 0) for v in MODEL_VARS}
...
# Fresh loads — never deep-copy `model` here, or the "FP32 baseline" becomes a
# copy of the quantized model and RMSE-vs-FP32 collapses to zero.
...
def get_gfs_truth(date_str):
    """GFS truth field at each verification time, for RMSE."""
...
for d in EVAL_DATES:
    print(f"  Eval {d}")
    gfs_truth    = get_gfs_truth(d)      # for RMSE
    era5_truth   = get_era5_truth(d)     # for ACC
    q_fore, leads= run_fcast(model,      d)
    f_fore, _    = run_fcast(fp32_model, d)
    clim_d       = get_climatology(d)
    for v in MODEL_VARS:
        if v not in gfs_truth: continue
        # RMSE metrics use GFS truth (self-consistent with GFS IC)
        re[v].append(lat_rmse(q_fore[v], gfs_truth[v]))
        r32[v].append(lat_rmse(f_fore[v], gfs_truth[v]))
        rf[v].append(lat_rmse(q_fore[v], f_fore[v]))
        # ACC uses ERA5 truth + WB2 ERA5 climatology (matched anomaly sources)
        if clim_d and v in clim_d and era5_truth and v in era5_truth:
            ae[v].append(lat_acc(q_fore[v], era5_truth[v], clim_d[v]))
            af[v].append(lat_acc(f_fore[v], era5_truth[v], clim_d[v]))
...
try: plot_panel(ae, af,  "Lat-wtd ACC (vs ERA5)", "acc", hline=0.6, ylim=(-0.1, 1.05))
