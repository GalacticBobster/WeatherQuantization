...
for cfg in CONFIGS:
    npys = sorted(glob.glob(
        os.path.join(BASE, "outputs_*", f"{MODEL}_bench", cfg, f"{MODEL}_{cfg}_acc.npy")))
    if not npys:
        print(f"  MISSING {cfg}")
        continue
...
def render(source_key, ylabel, tag, hline=None, ylim=None):
    ...
    for i, (ax, v) in enumerate(zip(axes, MODEL_VARS)):
        ...
        if i == 0: ax.legend(fontsize=8, loc="best")   # legend on first panel only
        n_plot += 1
...
render("re", f"Lat-wtd RMSE vs {TRUTH}",  "rmse_vs_truth")
render("rf", "Lat-wtd RMSE vs FP32",       "rmse_vs_fp32")
render("ae", "Lat-wtd ACC",                "acc", hline=0.6, ylim=(-0.1, 1.05))
